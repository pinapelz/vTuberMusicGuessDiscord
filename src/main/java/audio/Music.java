        ...
        if (!guessingPeriod && !event.getAuthor().isBot()) { //Checking for while the game is not currently running and user is not bot
            ...
            Runnable runnable = () -> {
                ...
                if (guessingPeriod) {
                    ...
                    event.getChannel().sendMessage("15 Seconds Remaining").queue();
                    try {
                        TimeUnit.SECONDS.sleep(5);
                    } catch (InterruptedException e) {
                        throw new RuntimeException(e);
                    }
                    event.getChannel().sendMessage("10 Seconds Remaining").queue();
                    try {
                        TimeUnit.SECONDS.sleep(5);
                    } catch (InterruptedException e) {
                        throw new RuntimeException(e);
                    }
                    event.getChannel().sendMessage("5 Seconds Remaining").queue();
                    try {
                        TimeUnit.SECONDS.sleep(5);
                    } catch (InterruptedException e) {
                        throw new RuntimeException(e);
